...
class Ventas():
	...
	def __init__(self,ventana_ventas):
        ...
        # Icono
		#self.ventana_ventas.iconbitmap("C:/Users/Aprendiz/Desktop/tienda/tiendaform_icon_142679.ico")
	...
	#--------------Menu de la ventana----------------------
		...
		self.frame_logo_productos = LabelFrame(ventana_ventas)
		...
		self.frame_buscar_producto = LabelFrame(ventana_ventas, text= "Buscar venta", font=("comic sans", 10, "bold"),pady=10)
		...
		titulo = Label(ventana_ventas, text="Registro de producto	", fg="black", font=("Arial", 10, "bold"));"""pack()"""

		frame_registro = LabelFrame(ventana_ventas, text="Datos del producto	", font=("Arial", 10, "bold"))
		...
		#----------------------------------------DATO 3-------------------------------------------------------------
		label_categoria = Label(self.frame_registro, text="tipo de pago: ",font=("Arial", 10, "bold")).grid(row=2, column=0,sticky='s',padx=5,pady=0)
		self.combo_categoria = ttk.Combobox(self.frame_registro, values=["credito", "contado"], width=22, state="readonly")
		...
		boton_editar = Button(self.frame_botones_registro, text="Editar",command=self.editar_ventas, height=2, width=9, bg="blue", fg="white", font=("Comic Sans MS",8,"bold")).grid(row=1, column=2, padx=10, pady=10)
		...
		boton_eliminar= Button(self.frame_botones_registro, text="Eliminar",command=self.eliminar_ventas, height=2, width=9, bg="orange", fg="white", font=("Comic Sans MS",8,"bold")).grid(row=1, column=5, padx=10, pady=10)

	#------------------------------Metodos----------------------------------------------------------------------------------------------------------------------
	def registrar_ventas(self):
		if self.validar_formulario() and self.validar_registrar():
			query = "INSERT INTO Ventas VALUES(NULL,?,?,?,?,?)"
			parametros = (self.codigo.get(),self.nombre.get(),self.combo_categoria.get(),self.cantidad.get(),self.precio.get())
	...
	def validar_formulario(self):
		if  len(self.codigo.get())!=0 and len(self.nombre.get())!=0 and len(self.combo_categoria.get())!=0 and len(self.cantidad.get())!=0 and len(self.precio.get())!=0:
			return True
		else:
	...
	def editar_ventas(self):
		...
		label_nombre = Label(self.ventana_editar, text="Nombre de la venta:", font=("Comic Sans", 10, "bold")).grid(row=1, column=0, sticky='s', padx=5, pady=8)
		...
		boton_actualizar = Button(self.ventana_editar, text="ACTUALIZAR", command=lambda:self.actualizar(nuevo_codigo.get(), nuevo_nombre.get(), nuevo_combo_categoria.get(), nueva_cantidad.get(), nuevo_precio.get(),  codigo), height=2, width=20, bg="blue", fg="white", font=("Comic Sans MS", 9, "bold"))
	...
	def actualizar(self, nuevo_codigo, nuevo_nombre, nuevo_combo_categoria, nueva_cantidad, nuevo_precio,  codigo):
		query = 'UPDATE Ventas SET codigo = ?, nombre = ?, categoria = ?, cantidad = ?, precio = ? WHERE codigo = ?'
		parameters = (nuevo_codigo, nuevo_nombre, nuevo_combo_categoria, nueva_cantidad, nuevo_precio, codigo)
	...
	def eliminar_ventas(self):
		...
		if respuesta == "yes":
			...
			messagebox.showerror('ERROR',F'Error al eliminar el venta: {nombre}')
	...
	def widgets_buscador(self):
		...
		self.label_codigo_nombre = Label(self.frame_buscar_producto, text= "Codigo / Nombre de la venta : ",font=("comic sans", 10, "bold")).grid(row=0,column=2,sticky='s',padx=5,pady=5)
